...
class BaseCalendar {
    ...
    fun changeToNextYear(refreshCallback: (Calendar) -> Unit) {
        calendar.set(Calendar.YEAR, calendar.get(Calendar.YEAR) + 1)
        makeMonthDate(refreshCallback)
    }

    fun changeToPrevYear(refreshCallback: (Calendar) -> Unit) {
        if (calendar.get(Calendar.YEAR) == 0) return    // ... 과연 0까지 내려보는 사람이 있나 ..
        calendar.set(Calendar.YEAR, calendar.get(Calendar.YEAR) - 1)
        makeMonthDate(refreshCallback)
    }
