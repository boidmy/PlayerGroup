...
import com.example.playergroup.custom.AppBarStateChangeListener.AppBarState.*
import com.example.playergroup.custom.EventObserver
...
class ClubActivity: BaseActivity<ActivityClubBinding>() {
    ...
    private fun initViewModel() {
        clubViewModel.apply {
            ...
            scrollToTopBtnStateChangeEvent.observe(this@ClubActivity, Observer {
                if (it) binding.moveToTop.show()
                else binding.moveToTop.hide()
            })
            scrollTopEvent.observe(this@ClubActivity, EventObserver {
                if (it == true) {
    ...
    private fun initView(clubInfo: ClubInfo) {
        ...
        binding.appBar.addOnOffsetChangedListener(object: AppBarStateChangeListener() {
            override fun onStateChanged(appBarLayout: AppBarLayout?, state: AppBarState) {
                clubViewModel.setViewTopMode(state)
            }
        })

        binding.moveToTop click {
            clubViewModel.setScrollToTop()
        }
    }
